...
from model.STDC.nets.stdcnet import STDCNet1446, STDCNet813
...
class ContextPath(nn.Module):
    def __init__(self, backbone='CatNetSmall', pretrain_model='', use_conv_last=False, input_size=512, *args, **kwargs):
        ...
        elif self.input_size == 360:
            self.H8 = torch.tensor(45)
            self.W8 = torch.tensor(60)

            self.H16 = torch.tensor(23)
            self.W16 = torch.tensor(30)

            self.H32 = torch.tensor(12)
            self.W32 = torch.tensor(15)
        else:
...
class BiSeNet(nn.Module):
    def __init__(self, backbone, n_classes, pretrain_model='', use_boundary_2=False, use_boundary_4=False, use_boundary_8=False, use_boundary_16=False, input_size=512, use_conv_last=False, heat_map=False, *args, **kwargs):
        ...
        if self.input_size == 512:
            ...
            self.H = torch.tensor(360)
            self.W = torch.tensor(480)
        else:
            print("input_size is not in input_size lists")
            exit(0)
        self.business_layer = []
        self.init_weight()
        self.business_layer.append(self.cp)
        self.business_layer.append(self.ffm)
        self.business_layer.append(self.conv_out)
        self.business_layer.append(self.conv_out_sp16)
